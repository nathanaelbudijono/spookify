...
import { PopularRadios } from "@/constant/music";

import { FaPlay } from "react-icons/fa6";
import { ScrollArea, ScrollBar } from "@/components/ui/scrollArea";

const PopularRadio = ({ panelSize }: { panelSize: number }) => {
  const [hoveredIndex, setHoveredIndex] = React.useState<number | null>(null);
  const [card, setCard] = React.useState<number>(5);
  const pRadio = PopularRadios;
  ...
  return (
    <main className="h-[75vh] max-xl:h-full p-4">
      <section className="flex justify-between">
        <Typography variant="h2">Popular radio</Typography>
      ...
      <ScrollArea className="w-full border-none whitespace-nowrap rounded-md border">
        <section className="mt-3 flex gap-4">
          {pRadio.slice(0, card).map((item, index: number | null) => {
            return (
              <div
                key={index}
                className="flex flex-col w-full bg-secondary-100/10 rounded-md p-4 hover:bg-secondary-100/50 duration-200 ease-in-out transition-colors"
